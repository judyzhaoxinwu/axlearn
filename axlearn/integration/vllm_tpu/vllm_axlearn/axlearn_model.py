...
def _recursive_sanitize_specs(cfg, allowed_axes, mha_cls=None, gqa_cls=None, moe_cls=None):
    ...
    if isinstance(cfg, ConfigBase):
        if isinstance(cfg, Repeat.Config):
            cfg.unroll = True
        if mha_cls and gqa_cls and isinstance(cfg, TransformerAttentionLayer.Config):
        ...
        for key in cfg.keys():
            ...
            if isinstance(val, dict):
                ...
                if isinstance(val, PartitionSpec):
                    cfg.set(**{key: _sanitize_partition_spec(val, allowed_axes)})
...
def _recursive_set_block_size(cfg):
    ...
    if isinstance(cfg, ConfigBase):
        if isinstance(cfg, FlashAttention.Config):
            logger.info("Setting tpu_block_size=128 to fit within TPU v6e VMEM limit")
...
class AxLearnForCausalLM(nnx.Module):
    def __init__(self, vllm_config: VllmConfig, rng_key: jax.Array, mesh: Mesh) -> None:
        ...
        with self.mesh:
            ...
            if _orig_apply_rotary_position_embeddings is None:
                _orig_apply_rotary_position_embeddings = (
                    axlearn_attention.apply_rotary_position_embeddings
                )
            ...
            axlearn_attention.apply_rotary_position_embeddings = (
                split_half_apply_rotary_position_embeddings
            )
            logger.info(
                "=== [MONKEY PATCH] === Successfully registered dynamic HuggingFace/AxLearn RoPE router!"
            )
            from axlearn.experiments.text.gpt.c4_trainer import named_trainer_configs as c4_configs
            ...
            class VllmAttentionMixin:
                def _forward_for_mode(
                    ...
                    *,
                    ...
                    _vllm_context.kv_caches[_vllm_context.layer_index] = new_kv_cache
                    _vllm_context.layer_index += 1

                    out = self.o_proj(outputs)
                    return dict(), self.Output(data=out)

            class VllmMultiheadAttention(VllmAttentionMixin, MultiheadAttention):
                pass
        ...
        if use_registry:
            ...
            num_kv_heads = getattr(model_config_hf, "num_key_value_heads", None)
            ...
            if num_kv_heads and num_kv_heads != model_config_hf.num_attention_heads:
                ...
                atten_input_linear = FusedGroupedQKVLinear.default_config().set(
                    num_kv_heads=num_kv_heads
                )
            else:
                atten_cfg = VllmMultiheadAttention.default_config().set(hidden_dim=atten_hidden_dim)
            ...
            num_experts = getattr(
                model_config_hf, "num_local_experts", getattr(model_config_hf, "num_experts", None)
            )
            ...
            self.axlearn_model_config = common_model_config(
                ...
                lm_head_cfg=None
                if getattr(model_config_hf, "tie_word_embeddings", False)
                else decoder.LmHead.default_config(),
        ...
        allowed_axes = abstract_mesh.axis_names if not abstract_mesh.empty else ("model", "expert")
        logger.info(
            f"Sanitizing model PartitionSpecs to match active compilation mesh axes: {allowed_axes}"
        )
        _recursive_sanitize_specs(
            self.axlearn_model_config,
            allowed_axes,
    ...
    def __call__(
        ...
        *args,
        ...
        List[jax.Array], jax.Array | JaxIntermediateTensors, List[jax.Array], Optional[jax.Array]
    ]:
    ...
    def compute_logits(self, hidden_states: jax.Array) -> jax.Array:
        ...
        with self.mesh:
            if hasattr(self.model.decoder, "lm_head") and self.model.decoder.lm_head is not None:
                logits, _ = F(
    ...
    def load_weights(self, rng_key: jax.Array):
        ...
        if "axlearn_config" in self.vllm_config.additional_config:
            ckpt_path = self.vllm_config.additional_config["axlearn_config"].get("ckpt_path", None)
        ...
        if ckpt_path is not None:
            ...
            with self.mesh:
                target_specs = dict(model=self.model.create_parameter_specs_recursively())
    ...
    def get_mrope_input_positions(
        self, prompt_token_ids: List[int], mm_features: List[Any]
    ) -> Tuple[jax.Array, int]:

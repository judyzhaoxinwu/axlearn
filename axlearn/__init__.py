...
try:
    ...
    print("[AxLearn Preloader] Successfully patched logical JAX mesh axis names with fsdp, expert, and seq!", file=sys.stderr)
except ImportError:
    pass
...
try:
    ...
    def _custom_mesh_new(cls, devices, axis_names, axis_types=None, *args, **kwargs):
        ...
        if len(axis_names) > devices.ndim and devices.size == 1:
            ...
            if axis_types is not None:
                ...
                axis_types = list(axis_types) + [AxisType.Auto] * (len(axis_names) - len(axis_types))
    ...
    print("[AxLearn Preloader] Successfully installed JAX Mesh dimension and axis_types auto-aligner!", file=sys.stderr)
